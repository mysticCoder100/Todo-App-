...
<x-base.base>
    <main class="grid items-center justify-items-center">
        <section class="[&>*]:p-6  w-[min(95%,30rem)]">
            <form action=""
                  class="card bg-base-100 border border-primary shadow-xl shadow-gray-300 dark:shadow-gray-900 rounded-xl p-6 max-w-md mx-auto"
                  method="POST">
                ...
                <div class="mb-4">
                    @foreach($registerFields as $registerField)
                        ...
                        @endphp
                        <livewire:input name="{{ $registerField['name'] }}" label="{{ $registerField['label'] }}"
                                        placeholder="{{ $registerField['placeholder'] }}"
                                        type="{{ $registerField['type'] }}" class="w-full {{ $style  }}"
