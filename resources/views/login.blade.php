...
<x-base.base>
    <main class="grid items-center justify-items-center">
        <section class="[&>*]:p-6  w-[min(95%,30rem)]">
            <form action=""
                  class="card bg-base-100 shadow-lg border border-primary shadow-gray-300 dark:shadow-gray-900 rounded-xl p-6 max-w-md mx-auto"
                  method="POST">
                ...
                <div class="mb-4">
                    @foreach($loginFields as $loginField)
                        ...
                        @endphp
                        <livewire:input name="{{ $loginField['name'] }}" label="{{ $loginField['label'] }}"
                                        placeholder="{{ $loginField['placeholder'] }}" type="{{ $loginField['type'] }}"
                                        class="w-full {{ $style  }}"
